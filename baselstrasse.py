# Baselstrasse.py
# Baselstrasse Co. LTD – Inventory & Service Cockpit (Streamlit app)
...
def planner_copilot_answer(
    ...
    if any(k in q for k in ("summary", "coo", "executive", "overview", "report")):
        return (
            "Executive summary for Baselstrasse Co. LTD:\n\n"
...
def main():
    st.set_page_config(
        page_title="Baselstrasse Inventory & Service Cockpit", layout="wide"
    )
    st.title("Baselstrasse Co. LTD – Inventory & Service Level Cockpit")
    ...
    with tab4:
        ...
        if st.button("Generate Carousel PDF", type="primary"):
            ...
            def add_slide(title, lines, badge=None):
                ...
                pdf.set_text_color(*MUTED)
                pdf.set_xy(8, 93)
                pdf.cell(86, 5, "Baselstrasse Co. LTD  |  Inventory & Service Cockpit")

            # Slide 1 — Cover
            add_slide(
                "Baselstrasse Inventory Overview ",
                [
